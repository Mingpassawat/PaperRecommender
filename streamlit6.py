...
features_scaled = scaler.fit_transform(subject_data.iloc[:, 28:])  # Adjust column range as needed
...
subject_data["Title"] = titles.values  # Add titles for hover information

# Create a hover text column with full title
subject_data["hover_text"] = subject_data["Title"].apply(
    lambda x: f"Research Name: {x}<br>PCA1: {{:.2f}}<br>PCA2: {{:.2f}}".format(0, 0)
)
